...
export default class Bushes extends Entities {
  constructor(reffs = []) {
    ...
    this._geometry = bushes.createGeometry(this._sections, 100)
    ...
    this._depthMap = new THREE.MeshDepthMaterial({
      depthPacking: THREE.RGBADepthPacking,
      alphaTest: .5,
      transparent: true
    })
    this._depthMap.onBeforeCompile = (shader) => {
      shader.uniforms = { ...shader.uniforms, ...this._uniforms }
      shader.vertexShader = shader.vertexShader.replace('#include <common>', foliagesVertPar)
      shader.vertexShader = shader.vertexShader.replace('#include <begin_vertex>', foliagesVertMain)
    }
    ...
    this._mesh = new THREE.InstancedMesh(this._geometry, this._material, this._reffs.length)
    this._mesh.customDepthMaterial = this._depthMap
    this._mesh.position.copy(this._position)
  }

  registerDebugger() {
    const f = this.Debug.ui.addFolder({ title: 'bushes' })
    f.addBinding(this._config, 'color').on('change', () => {
      this._mesh.material.color.copy(new THREE.Color(this._config.color))
    })
  }
